...
class Flight:
    ...
    def calculate_delays(self):
        ...
        scheduled_time = datetime.strptime(self.scheduled_arrival_time, "%H:%M")
        estimated_time = self.calculate_estimated_arrival_time()
        delays = estimated_time - scheduled_time
        return delays
...
def FlightMenu(flight):
    ...
    print(f"  - Estimated Arrival Time: {flight.calculate_estimated_arrival_time().strftime('%H:%M')}")
    print(f"  - Delays: {flight.calculate_delays()}")
    ...
    if option == 1:
        while True:
            ...
            if sub_choice == '1':
                ...
                print(f"Flight {flight1.flight_number} to {flight1.flight_origin} at {flight1.calculate_estimated_arrival_time().strftime('%H:%M')}")
                print(f"Flight {flight2.flight_number} to {flight2.flight_origin} at {flight2.calculate_estimated_arrival_time().strftime('%H:%M')}")
                print(f"Flight {flight3.flight_number} to {flight3.flight_origin} at {flight3.calculate_estimated_arrival_time().strftime('%H:%M')}")
                print(f"Flight {flight4.flight_number} to {flight4.flight_origin} at {flight4.calculate_estimated_arrival_time().strftime('%H:%M')}")
                print(f"Flight {flight5.flight_number} to {flight5.flight_origin} at {flight5.calculate_estimated_arrival_time().strftime('%H:%M')}")
            elif sub_choice == '5':
                f = open("Fly.txt", "r")
                print(f.readline())
            elif sub_choice == '6':
                print(f"Flight {flight1.flight_number} Delays: {flight1.calculate_delays()}")
                print(f"Flight {flight2.flight_number} Delays: {flight2.calculate_delays()}")
                print(f"Flight {flight3.flight_number} Delays: {flight3.calculate_delays()}")
                print(f"Flight {flight4.flight_number} Delays: {flight4.calculate_delays()}")
                print(f"Flight {flight5.flight_number} Delays: {flight5.calculate_delays()}")
            else:
